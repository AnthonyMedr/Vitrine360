...
type QuoteCartContextValue = {
  ...
  totalItems: number;
  totalLines: number;
  addProduct: (product: Product, options?: { quantity?: number; notes?: string | null }) => void;
  updateItem: (
    id: string,
    patch: Partial<Pick<QuoteCartItem, "quantity" | "unit" | "notes">>,
  ) => void;
...
function normalizeQuantity(value: unknown) {
  ...
  return Math.min(Math.trunc(quantity), MAX_QUOTE_ITEM_QUANTITY);
}
...
function sanitizeStoredItem(value: unknown): QuoteCartItem | null {
  ...
  return {
    id: String(item.id || `${productId}:default`),
    productId,
...
function readStoredItems() {
  ...
  try {
    ...
    return rawItems
      ...
      .filter(Boolean)
      .slice(0, MAX_QUOTE_CART_ITEMS) as QuoteCartItem[];
  } catch {
...
function buildItem(
  product: Product,
  options?: { quantity?: number; notes?: string | null },
): QuoteCartItem {
  return {
...
export function QuoteCartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const syncAcrossTabs = (event: StorageEvent) => {
      if (event.key === QUOTE_CART_STORAGE_KEY) setItems(readStoredItems());
    };
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem(
      ...
      JSON.stringify({ version: QUOTE_CART_STORAGE_VERSION, items, expiresAt: null }),
    );
  }, [hydrated, items]);

  const addProduct = useCallback(
    (product: Product, options?: { quantity?: number; notes?: string | null }) => {
  ...
  const updateItem = useCallback(
    (id: string, patch: Partial<Pick<QuoteCartItem, "quantity" | "unit" | "notes">>) => {
      setItems((current) =>
        current.map((item) =>
          item.id === id
            ? {
                ...
                notes: patch.notes === undefined ? item.notes : patch.notes?.slice(0, 500) || null,
              }
            : item,
        ),
      );
